...
export default function RadioTypeActionable({
  ...
  const matchesRule = (rule: Rule, val: string) => {
    ...
    switch (rule.conditionType) {
      ...
      case "regex":
        try { return new RegExp(v).test(val); } catch { return false; }
      default:
        return false;
    }
  };
  ...
  // trigger / modal (same UI pattern as InputTypeActionable)
  const trigger = (
    <button
      ref={triggerRef}
      data-no-drag="true"
  ...
  const modal = open
    ? createPortal(
        <div className="fixed inset-0 flex items-center justify-center" style={{ zIndex: 99999 }} onMouseDown={closeModal} role="dialog" aria-modal="true">
          ...
          <div role="document" onMouseDown={(e) => e.stopPropagation()} style={{ position: "relative", zIndex: 100000, width: 720, maxWidth: "96%", maxHeight: "86vh", overflow: "auto", background: "#fff", borderRadius: 8, padding: 16, boxShadow: "0 10px 30px rgba(0,0,0,0.25)" }} data-no-drag="true">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  <select value={operator} onChange={(e) => setOperator(e.target.value === "AND" ? "AND" : "OR")} data-no-drag="true" style={{ padding: "6px 8px", borderRadius: 6, color:"#333" }}>
            ...
            <p style={{ color: "#555", marginBottom: 12 }}>Add rules to make this radio group actionable.</p>

            <div style={{ marginBottom: 12 }}>
              <button onClick={(e) => { e.stopPropagation(); addRule(); }} data-no-drag="true" style={{ padding: "6px 10px", fontSize: 13, color:"#333" }}>+ Add Rule</button>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              {rules.map((rule, idx) => (
                <div key={idx} style={{ border: "1px solid #e5e7eb", borderRadius: 6, padding: 12 , color:"#333"}}>
                  ...
                  <div style={{ color: "#777", fontSize: 12, marginTop: 8 }}>Tip: "like" supports % as wildcard — regex uses JS RegExp (e.g. ^hello).</div>
                </div>
              ))}
            </div>
          </div>
  ...
  // Render trigger & modal. Rendering of radio inputs in RenderComponent should call evaluateAndDispatch on change.
  return (
    <>
      {trigger}
      {modal}
